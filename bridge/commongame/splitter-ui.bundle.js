...
function highCardPoints(hand) {
    var rankToPoints = function (rank) { return Math.max(0, rank - 10); };
    var hcp = 0;
    _.each(hand, function (nums) {
        nums.forEach(function (num) { hcp += rankToPoints(num); });
    });
    return hcp;
}
function ddUrl(deal) {
...
function dealsToHTML(deals, headerHTML) {
    headerHTML = headerHTML || '';
    var out = {
        N: headerHTML + "<h1>North</h1><div class=\"player\">",
        S: headerHTML + "<h1>South</h1><div class=\"player\">",
        E: headerHTML + "<h1>East</h1> <div class=\"player\">",
        W: headerHTML + "<h1>West</h1> <div class=\"player\">",
    };
    var _loop_1 = function(deal) {
        ...
        PLAYERS.forEach(function (player) {
            ...
            var hcp = highCardPoints(hand[player]);
    ...
    _.each(out, function (html, player) {
...
exports.dealsToHTML = dealsToHTML;

},{"underscore":3}],5:[function(require,module,exports){
// Todo:
// - support drag & drop of PBN files
// - link directly to a date
"use strict";
var pbn_splitter_1 = require('./pbn-splitter');
...
function yymmdd(date) {
    ...
    var mm = pad(1 + date.getMonth());
    var dd = pad(date.getDate());
    return "" + yy + mm + dd;
}
function commonGamePbnUrl(date) {
    return "http://thecommongame.com/PBN/" + yymmdd(date) + "HandRecord.pbn";
}
function commonGameUrl(date) {
    return "http://thecommongame.com/Results/20" + yymmdd(date) + "Rank.html";
}
function fetchPBN(date) {
    var url = commonGamePbnUrl(date);
    var corsUrl = "https://crossorigin.me/" + url;
    return fetch(corsUrl).then(function (response) {
        if (!response.ok) {
            // XXX rather than throwing.
...
function handlePickDate(date) {
    ...
    fetchPBN(date).then(function (pbn) {
        var linkHTML = "<a style=\"float: right; color: black;\" href=\"" + commonGameUrl(date) + "\">" + date.toLocaleDateString() + "</a>";
        var deals = pbn_splitter_1.loadDeals(pbn);
        var html = pbn_splitter_1.dealsToHTML(deals, linkHTML);
        document.getElementById('selector').style.display = 'none';
        var outputEl = document.getElementById('output');
        outputEl.style.display = 'block';
        outputEl.innerHTML = html;
        // window.location.hash = yymmdd(date);
    }, function (response) {
        errorEl.style.display = 'block';
        errorEl.textContent =
            "Unable to load boards. Error: " + response.status + " " + response.statusText;
    }).then(function () {
        loadingEl.style.display = 'none';
    });
}
function main() {
    var minDate = new Date(2012, 3, 2);
    var yesterday = new Date(new Date().getTime() - 86400000);
    var picker = new Pikaday({
        defaultDate: yesterday,
        setDefaultDate: true,
        field: document.getElementById('datepicker-text'),
        firstDay: 0,
        minDate: minDate,
        maxDate: yesterday,
        yearRange: [minDate.getFullYear(), yesterday.getFullYear()],
        bound: false,
